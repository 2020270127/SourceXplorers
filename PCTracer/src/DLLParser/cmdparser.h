...
namespace cmdparser
{
    ...
    public:
        ...
        virtual bool compareOptionName(const tstring& name) const;
        virtual bool isRequiredOption(void) const;
        virtual bool isOptionSet(void) const;
    ...
    private:
        ...
        vector<CmdBase*>::iterator getOptionIteratorByName(const tstring& name);

    public:
        CmdParser() : help_(false) {};
        ~CmdParser() { clearCommandVector(); };
        void clearCommandVector(void);
