...
namespace cmdparser
{
    ...
    bool CmdBase::compareOptionName(const tstring& name) const
    {
        return ((name_.compare(name) == 0) || (alternative_.compare(name) == 0));
    };

    // 필수 옵션인지 여부 확인
    bool CmdBase::isRequiredOption(void) const
    {
        return required_;
    };

    // 옵션 값이 설정되었는 지 여부 확인
    bool CmdBase::isOptionSet(void) const
    {
        return isSet_;
    };
    ...
    vector<CmdBase*>::iterator CmdParser::getOptionIteratorByName(const tstring& name)
    {
        for (vector<CmdBase*>::iterator iter = command_.begin(); iter != command_.end(); iter++)
        {
            if ((*iter)->compareOptionName(name))
            {
                return iter;
            }
        }
    ...
    // 저장된 옵션 모두 제거
    void CmdParser::clearCommandVector
    (void)
    {
        for (const auto& iter : command_)
        {
        ...
        else
        {
            ...
            for (int index = 1; index < argc; index++)
            {
                ...
                else if (name.at(0) == '-')
                {
                    ...
                    vector<CmdBase*>::iterator iter = getOptionIteratorByName(name);
